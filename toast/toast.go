...
func init() {
	toastTemplate = template.New("toast") // activationType="{{.ActivationType}}" launch="{{.ActivationArguments}}" duration="{{.Duration}}"
	toastTemplate.Parse(`
...
<toast activationType="{{.ActivationType}}" launch="{{.ActivationArguments}}" duration="{{.Duration}}" {{if .Scenario}}scenario="{{.Scenario}}"{{end}}>
    ...
    <actions>
        ...
        <!-- Системные элементы для reminder (обязательны) -->
